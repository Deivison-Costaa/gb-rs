...
- [x] 3.1 Registradores: LCDC, STAT, SCY, SCX, LY, LYC, BGP, OBP0, OBP1, WY, WX. VRAM/OAM.
...
- [ ] 6.7 Downsample para 48 kHz + ring buffer + saída via `cpal`.
  - [ ] 6.7a Acumulador de downsample no `Apu` (gb-core): soma amostras do mixer a cada M-cycle, divide pelo número de M-cycles acumulados a cada ~87, produzindo `(f32, f32)` a ~48 kHz. Inclui ring buffer circular exposto como `&[(f32, f32)]` para consumo pelo `gb-desktop`. Testes unitários.
