...
**Conserto fora da escada:** 0077d — DMA de OAM e prioridade BG-over-OBJ ([doc](docs/iterations/0077d-oam-dma.md)). `$FF46` era stub desde o 3.1b: escrita guardava o valor e nada era transferido, então **jogo comercial rodava sem um único sprite** (a OAM ficava zerada e `Y=0` põe todo objeto fora da tela). Agora `Bus::tick_dma()` copia 160 bytes, um por M-cycle, com barramento próprio; OAM inacessível à CPU durante a transferência. Junto: a prioridade BG-over-OBJ comparava o shade pós-BGP em vez do índice de cor do BG — a PPU passou a guardar `bg_color` por scanline. 8 testes novos. Placar inalterado (18/121) e hash do dmg-acid2 idêntico — nenhuma ROM da bateria exercita `$FF46`. **Limitação assumida:** o conflito de barramento do DMG (durante a DMA a CPU só enxerga HRAM) não foi implementado.
